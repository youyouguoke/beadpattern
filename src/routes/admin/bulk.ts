...
bulk.get('/jobs', async (c) => {
  const db = getDB(c.env);
  const page = Number(c.req.query('page') ?? 1);
  const limit = Number(c.req.query('limit') ?? 20);
  const offset = (page - 1) * limit;
  const countRow = await db.queryOne<{ count: number }>('SELECT COUNT(*) as count FROM bulk_jobs');
  const total = countRow?.count ?? 0;
  const rows = await db.query('SELECT * FROM bulk_jobs ORDER BY created_at DESC LIMIT ? OFFSET ?', [limit, offset]);
...
bulk.post('/import/preview', async (c) => {
  const db = getDB(c.env);
...
bulk.post('/import/create', async (c) => {
  const db = getDB(c.env);
  const body = await c.req.json<{ source_type: string; source_data: string }>();
  const rawRows = parseInput(body);
  ...
  return c.json(success({ jobId, total: rawRows.length, processed, failed, errors }), 201);
});

bulk.post('/import/publish', async (c) => {
  const db = getDB(c.env);
