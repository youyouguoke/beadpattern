...
media.get('/:id/references', async (c) => {
  const db = getDB(c.env);
  const id = c.req.param('id');
  const mediaRow = await db.queryOne<Media>('SELECT id FROM media WHERE id = ?', [id]);
  if (!mediaRow) throw new AppError('Media not found', 'MEDIA_NOT_FOUND', 404);

  const references: { type: string; pattern_id: string; pattern_slug: string; pattern_title: string }[] = [];

  const coverPatterns = await db.query<{ id: string; slug: string; title: string }>(
    'SELECT id, slug, title FROM patterns WHERE cover_media_id = ?',
    [id]
  );
  for (const p of coverPatterns) {
    references.push({ type: 'cover', pattern_id: p.id, pattern_slug: p.slug, pattern_title: p.title });
  }

  const finishedPatterns = await db.query<{ id: string; slug: string; title: string }>(
    'SELECT id, slug, title FROM patterns WHERE finished_media_id = ?',
    [id]
  );
  for (const p of finishedPatterns) {
    references.push({ type: 'finished', pattern_id: p.id, pattern_slug: p.slug, pattern_title: p.title });
  }

  const galleryPatterns = await db.query<{ id: string; slug: string; title: string; gallery_media_ids: string }>(
    'SELECT id, slug, title, gallery_media_ids FROM patterns WHERE gallery_media_ids IS NOT NULL',
    []
  );
  for (const p of galleryPatterns) {
    try {
      const ids = JSON.parse(p.gallery_media_ids) as string[];
      if (ids.includes(id)) {
        references.push({ type: 'gallery', pattern_id: p.id, pattern_slug: p.slug, pattern_title: p.title });
      }
    } catch {
  ...
  const stepPatterns = await db.query<{ id: string; slug: string; title: string; step_media_ids: string }>(
    'SELECT id, slug, title, step_media_ids FROM patterns WHERE step_media_ids IS NOT NULL',
    []
  );
  for (const p of stepPatterns) {
    try {
      const ids = JSON.parse(p.step_media_ids) as string[];
      if (ids.includes(id)) {
        references.push({ type: 'step', pattern_id: p.id, pattern_slug: p.slug, pattern_title: p.title });
      }
    } catch {
      // ignore malformed JSON
    }
  ...
  return c.json(success(references));
});
...
media.put('/:id', async (c) => {
  ...
  if (body.used_by !== undefined) updates.used_by = JSON.stringify(body.used_by);
