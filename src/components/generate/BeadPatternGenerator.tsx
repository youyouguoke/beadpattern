...
import { Pattern, PatternDetail } from "@/types";
import { adminService } from "@/lib/adminService";
import { getPatternBySlug, getPublishedPatterns } from "@/lib/publicApiService";
...
function difficultyClass(diff: string) {
  ...
  if (normalized === "easy") return "bg-tertiary-container text-white";
  if (normalized === "hard") return "bg-error-container text-on-error-container";
...
export default function BeadPatternGenerator() {
  ...
  const [resultPattern, setResultPattern] = useState<PatternDetail | null>(null);
  ...
  const fetchRecent = async () => {
    const ps = await getPublishedPatterns({ limit: 4, sort: "newest" });
    setRecentPatterns(ps);
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const paletteItems = palette.map((hex) => ({ hex, name: hex, count: 0, code: "" }));
      const created = await adminService.createPattern({
        title: prompt,
        ...
        tagSlugs: ["ai-generated"],
      });
      if (!created?.id) throw new Error("Failed to create pattern");
      const detail = await adminService.getPattern(created.id);
      if (!detail) throw new Error("Pattern created but not returned");
      setResultPattern(detail);
      setGenerated(true);
      await fetchRecent();
    } catch (err) {
      ...
      <div className="lg:col-span-4 space-y-6">
        ...
        <div className="bg-white rounded-2xl p-6 border border-secondary-container shadow-sm space-y-6">
          ...
          <div className="space-y-3">
            ...
            <p className="text-sm text-secondary">{selectedPalette.name}</p>
          </div>
              ...
              <div className="w-full space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center text-sm">
                  ...
                  <div className="bg-surface-container rounded-xl p-4 border border-secondary-container">
                    ...
                    <p className="text-primary font-headline-md">{resultPattern.gridSize} Grid</p>
                  </div>
                  <div className="bg-surface-container rounded-xl p-4 border border-secondary-container">
                ...
                <div className="bead-grid p-6 rounded-2xl bg-white flex justify-center">
                  <BeadRenderer
                    ...
                    palette={resultPattern.colorPalette?.map((c) => c.hex)}
                    width={400}
                    height={400}
          ...
          {recentPatterns.map((item) => (
            <Link key={item.slug} href={`/pattern/${item.slug}`} className="bg-white rounded-2xl p-4 border border-secondary-container flex items-center justify-between hover:-translate-y-1 transition-transform">
              <div>
                ...
                <p className="text-secondary text-sm">{item.gridSize} &bull; {item.colorCount} colors</p>
              </div>
              <span className={`text-xs font-bold px-2 py-1 rounded-full ${difficultyClass(item.difficulty)}`}>
