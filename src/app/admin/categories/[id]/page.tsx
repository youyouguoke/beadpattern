  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="font-display-md text-2xl text-primary">{id === "new" ? "Create Category" : "Edit Category"}</h1>
        <div className="flex gap-2">
          <button onClick={() => router.push("/admin/categories")} className="px-4 py-2 rounded-xl text-sm border border-outline-variant/20 hover:bg-surface-container-low-container">Cancel</button>
      ...
      <div className="bg-surface-container-lowest rounded-3xl border border-outline-variant/20 p-6 space-y-4 max-w-2xl">
        ...
        <div>
          <label className="text-sm font-medium text-on-surface-variant">Icon</label>
          <input className="w-full mt-1 border border-outline-variant/20 rounded-xl px-3 py-2 text-sm" placeholder="Material icon name, e.g. pets" value={item.icon || ""} onChange={(e) => setItem({ ...item, icon: e.target.value })} />
        </div>
        <div>
