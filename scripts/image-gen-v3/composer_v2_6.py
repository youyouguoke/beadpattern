...
ANATOMY_PRESETS: Dict[str, AnatomyProfile] = {
    ...
    'fox': AnatomyProfile(
        animal='fox', head_shape='triangular', head_rx=13, head_ry=10, head_cy=0.32,
        ear_shape='pointed', ear_rx=6, ear_ry=8, ear_offset_y=2,
        snout_shape='pointed', snout_rx=6, snout_ry=5, snout_offset_y=2,
    ...
    'penguin': AnatomyProfile(
        animal='penguin', head_shape='round', head_rx=9, head_ry=8, head_cy=0.28,
        ear_shape='none', ear_rx=0, ear_ry=0, ear_offset_y=0,
        snout_shape='beak', snout_rx=3, snout_ry=2, snout_offset_y=0,
        body_shape='oval', body_rx=10, body_ry=14, body_cy=0.60,
...
class SilhouetteComposer:
    ...
    def draw_head(self, cx: int, cy: int) -> None:
        ...
        a = self.a
        # Shadow outline
        if a.head_shape == 'triangular':
            # Triangle-ish head with rounded chin
            self._draw_triangle_head(cx, cy, a.head_rx + 1, a.head_ry + 1, 'S')
            self._draw_triangle_head(cx, cy, a.head_rx, a.head_ry, base)
        elif a.head_shape == 'oval':
            self._rot(cx, cy, a.head_rx + 1, a.head_ry + 1, self.s.body_rotation, 'S')
            self._rot(cx, cy, a.head_rx, a.head_ry, self.s.body_rotation, base)
        else:  # round
            self._rot(cx, cy, a.head_rx + 1, a.head_ry + 1, self.s.body_rotation, 'S')
            self._rot(cx, cy, a.head_rx, a.head_ry, self.s.body_rotation, base)
    ...
    def draw_body(self, cx: int, cy: int) -> None:
        base = self._base()
        a = self.a
        if a.body_shape == 'pear':
            # Upper narrow, lower wide
            self._draw_pear(cx, cy, a.body_rx + 1, a.body_ry + 1, 'S')
            self._draw_pear(cx, cy, a.body_rx, a.body_ry, base)
        elif a.body_shape == 'slim':
            self._rot(cx, cy, a.body_rx + 1, a.body_ry + 1, self.s.body_rotation, 'S')
            self._rot(cx, cy, a.body_rx, a.body_ry, self.s.body_rotation, base)
    ...
    def draw_ears(self, cx: int, head_cy: int) -> None:
        ...
        for side in [-1, 1]:
            ...
            if a.ear_shape == 'long':
                ...
                self._draw_tapered_ear(ecx, ecy, a.ear_rx, a.ear_ry, side, ear_base)
                self._draw_tapered_ear(ecx, ecy, a.ear_rx - 1, a.ear_ry - 2, side, 'W')
                self._draw_tapered_ear(ecx, ecy + 1, a.ear_rx - 2, a.ear_ry - 4, side, 'Q')
                self.comp.set(ecx, ecy - a.ear_ry, 'Z')
            elif a.ear_shape == 'pointed':
                # Triangle/cat/fox ear
                self._draw_triangle_ear(ecx, ecy, a.ear_rx, a.ear_ry, side, ear_base)
                self._draw_triangle_ear(ecx, ecy, a.ear_rx - 2, a.ear_ry - 2, side, 'Q')
                self.comp.set(ecx, ecy - a.ear_ry, 'Z')
            elif a.ear_shape == 'floppy':
                # Dog floppy ear hanging down
                self.comp.fill_ellipse(ecx, ecy + a.ear_ry // 2, a.ear_rx, a.ear_ry, ear_base)
                self.comp.fill_ellipse(ecx, ecy + a.ear_ry // 2, a.ear_rx - 1, a.ear_ry - 2, 'Q')
                self.comp.set(ecx, ecy + a.ear_ry, 'Z')
            elif a.ear_shape == 'round':
                # Bear/panda round ear
                self.comp.fill_ellipse(ecx, ecy, a.ear_rx, a.ear_ry, ear_base)
                self.comp.fill_ellipse(ecx, ecy, a.ear_rx - 2, a.ear_ry - 2, 'Q')
                self.comp.set(ecx, ecy - a.ear_ry + 1, 'Z')
                # Panda ears: solid black outer, smaller white inner
                if a.animal == 'panda':
                    self.comp.fill_ellipse(ecx, ecy, a.ear_rx - 1, a.ear_ry - 1, 'K')
                    self.comp.fill_ellipse(ecx, ecy, a.ear_rx - 3, a.ear_ry - 3, 'W')
    ...
    def draw_penguin_parts(self, cx: int, body_cy: int) -> None:
        a = self.a
        # White belly oval (smaller)
        self.comp.fill_ellipse(cx, body_cy + 1, max(3, a.body_rx - 3), max(3, a.body_ry - 4), 'W')
        # Flippers - wider, attached to body sides
    ...
    def compose(self) -> LayerComposer:
        ...
        body_cy = int(CANVAS_H * a.body_cy)

        # Body
        self.draw_body(cx, body_cy)
        if a.animal == 'penguin':
...
class FeaturePainter:
    ...
    def paint(self, head_cy: int) -> None:
        ...
        if self.s.material_style == 'mini':
            ...
            if eye_type not in ('big_round', 'sparkly', 'sleepy', 'wink'):
                eye_type = 'big_round'
...
def compose_animal_v2_6(skeleton: CharacterSkeleton, style: dict) -> List[List[str]]:
    comp = make_canvas()
...
GOLDEN_SAMPLES: List[Tuple[str, str, str, dict]] = [
    ...
    ('panda', 'sitting', 'panda-sitting', {'expression': 'cute', 'whiskers': False}),
    ('panda', 'eating', 'panda-eating', {'expression': 'happy', 'whiskers': False}),
    ...
    ('panda', 'hugging', 'panda-hugging', {'expression': 'love', 'whiskers': False}),
    ('penguin', 'standing', 'penguin-standing', {'expression': 'cute', 'whiskers': False}),
    ('penguin', 'waving', 'penguin-waving', {'expression': 'happy', 'whiskers': False}),
    ('penguin', 'lying', 'penguin-lying', {'expression': 'sleepy', 'whiskers': False}),
    ('penguin', 'hugging', 'penguin-hugging', {'expression': 'love', 'whiskers': False}),
    ('penguin', 'sleeping', 'penguin-sleeping', {'expression': 'sleepy', 'whiskers': False}),
]
...
def run_golden_samples():
    for animal, pose, slug, style in GOLDEN_SAMPLES:
        ...
        grid = compose_animal_v2_6(skeleton, style)
