...
export function renderGrid(grid: number[][], palette: Palette, options: { beadSize?: number; gap?: number; bgColor?: string } = {}) {
  const beadSize = options.beadSize ?? 16;
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      if (colorIndex === 0) {
        ...
        ctx.fillRect(px, py, beadSize, Math.floor(beadSize / 4));
        // bead shadow
        ctx.fillStyle = 'rgba(0,0,0,0.15)';
        ctx.fillRect(px, py + beadSize - Math.floor(beadSize / 4), beadSize, Math.floor(beadSize / 4));
      }
    }
  }
...
export function gridToSvg(grid: number[][], palette: Palette, cellSize = 20): string {
  const rows = grid.length;
  const cols = grid[0].length;
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      const swatch = palette.swatches[colorIndex - 1];
      svg += `\u003crect x="${x * cellSize}" y="${y * cellSize}" width="${cellSize}" height="${cellSize}" fill="${swatch.hex}" stroke="rgba(0,0,0,0.08)" stroke-width="1" /\u003e\n`;
    }
  }
  svg += '\u003c/svg\u003e';
  return svg;
