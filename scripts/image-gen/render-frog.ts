...
const OUT_DIR = path.join(__dirname, 'data', 'frog-render');

async function main() {
  fs.mkdirSync(OUT_DIR, { recursive: true });
  const refPath = path.join(__dirname, 'data', 'reference', 'frog.jpg');
  ...
  const { cover, finished, thumbnail } = renderer.render(gridOutput, { background: '#ffffff', gridSize: `${grid[0].length}x${grid.length}` }, 'Frog');
