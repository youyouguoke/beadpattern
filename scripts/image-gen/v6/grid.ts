...
export class Grid {
  ...
  fillRect(x: number, y: number, w: number, h: number, colorIndex: number) {
    for (let dy = 0; dy < h; dy++) {
      for (let dx = 0; dx < w; dx++) this.set(x + dx, y + dy, colorIndex);
    }
  }
  ...
  outlineRect(x: number, y: number, w: number, h: number, colorIndex: number) {
    for (let dx = 0; dx < w; dx++) { this.set(x + dx, y, colorIndex); this.set(x + dx, y + h - 1, colorIndex); }
    for (let dy = 1; dy < h - 1; dy++) { this.set(x, y + dy, colorIndex); this.set(x + w - 1, y + dy, colorIndex); }
  }
  ...
  mirrorX(cx: number, x: number, y: number, colorIndex: number) {
    this.set(x, y, colorIndex);
    this.set(cx + (cx - x), y, colorIndex);
  }
