...
// bg distance mask
let mask=[];
for(let y=0;y<h;y++){ const row=[]; for(let x=0;x<w;x++){ const idx=(y*w+x)*4; const p={r:data[idx],g:data[idx+1],b:data[idx+2],a:data[idx+3]}; const d=Math.abs(p.r-bg.r)+Math.abs(p.g-bg.g)+Math.abs(p.b-bg.b); row.push(p.a>128&&d>35); } mask.push(row); }
...
await sharp(out,{raw:{width:ps,height:ps,channels:4}}).png().toFile('/tmp/panda2-mask-preview.png');
console.log('Saved /tmp/panda2-mask-preview.png');
